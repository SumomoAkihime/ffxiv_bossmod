...
class MiasmaBreath(BossModule module) : Components.Cleave(module, (uint)AID.MiasmaBreath, new AOEShapeCone(13, 45.Degrees()), activeWhileCasting: false);
...
class Poison(BossModule module) : BossComponent(module)
{
    ...
    private sealed class PoisonUnsafeZone(WPos center, int poisonStage) : ShapeDistance
    {
        public override bool Contains(in WPos p) => !Safe(center, poisonStage, p);
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        if (_poisonStage > 0)
        {
            Arena.ZoneCircle(Arena.Center, 8, Colors.SafeFromAOE);
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                if (_poisonStage == 1)
                {
                    Arena.ZoneRect(Arena.Center + new WDir(0, 31.87f).Rotate(deg), deg, 17.675f, 17.675f, 3.9f, Colors.SafeFromAOE);
                }

                if (_poisonStage == 2)
                {
                    Arena.ZoneRect(Arena.Center + new WDir(0, 18.02f).Rotate(deg), deg, 2.95f, 2.95f, 2.95f, Colors.SafeFromAOE);
                    Arena.ZoneRect(Arena.Center + new WDir(0, 32.13f).Rotate(deg), deg, 2.95f, 2.95f, 2.95f, Colors.SafeFromAOE);
                    Arena.ZoneRect(Arena.Center + new WDir(0, 45.745f).Rotate(deg), deg, 2.95f, 2.95f, 2.95f, Colors.SafeFromAOE);
                }
            }
        }
    }
    ...
    {
        if (_poisonStage > 0)
            hints.AddForbiddenZone(new PoisonUnsafeZone(Arena.Center, _poisonStage), DateTime.MaxValue);
    }
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 92, NameID = 706)]
