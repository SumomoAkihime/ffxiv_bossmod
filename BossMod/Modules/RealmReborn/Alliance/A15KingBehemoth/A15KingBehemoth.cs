...
class Comet(BossModule module) : BossComponent(module)
{
    ...
    {
        if (_comets[slot] > WorldState.CurrentTime)
            hints.AddForbiddenZone(new SDCircle(Module.PrimaryActor.Position, Module.PrimaryActor.HitboxRadius + 2.4f), _comets[slot]);
    }
}

class EclipticMeteor(BossModule module) : Components.CastLineOfSightAOE(module, (uint)AID.EclipticMeteor, 100, false)
{
    public override ReadOnlySpan<Actor> BlockerActors()
        => Module.Enemies((uint)OID.Comet).Where(a => !a.Position.InCircle(Module.PrimaryActor.Position, Module.PrimaryActor.HitboxRadius + a.HitboxRadius)).ToArray();
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 92, NameID = 727)]
