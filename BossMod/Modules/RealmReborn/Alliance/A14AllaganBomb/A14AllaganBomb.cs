...
class BossPrio(BossModule module) : Components.GenericInvincible(module)
{
    protected override ReadOnlySpan<Actor> ForbiddenTargets(int slot, Actor actor)
        => Module.Enemies((uint)OID.Vassago).Any(v => !v.IsDead) ? new Actor[] { Module.PrimaryActor } : Array.Empty<Actor>();
}
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 92, NameID = 1873)]
