...
以下任务来自 RebornCN，导入时均标记为 `Contributed`。编号和名称不能用于跨仓身份判断，必须以 `GroupID=1093` 和主 `OID` 为准。

| 主 OID | RebornCN 模块 |
|---|---|
| `0x4BB8` | `CE211DoubledTrouble` |
| `0x4BBE` | `CE204AlabasterGuardian` |
| `0x4BC1` | `CE209WhatGoesAround` |
| `0x4BC5` | `CE208AwakenedHydra` |
| `0x4BCA` | `CE215ManyMouthsToFeed` |
| `0x4BD3` | `CE214ForbiddenFolios` |
| `0x4BD9` | `CE201FamiliarTactics` |
| `0x4BE1` | `CE207IslandKidnapper` |
| `0x4C46` | `CE212CursedResurgence` |
| `0x4C4B` | `CE205GluttonousCursefiend` |
| `0x4C4F` | `CE206RebelliousFamiliar` |
| `0x4C6D` | `CE202TinyTerror` |
| `0x4C77` | `CE210AcceptNoImitators` |
| `0x4D8F` | `CE203AppallingBehavior` |
| `0x4DFA` | `CE213WebOfTerror` |

同时导入公共辅助 `CriticalEngagement/ReplayValidatedCastAOEs.cs`。`CE206RebelliousFamiliar` 需要本仓最小兼容类型 `SDKnockbackInAABBSquareTowardsOrigin`。

RebornCN 源码中的 77 处集合容量语法 `[with(n)]` 已按本仓编译器要求等价转换为 `new(n)`；该适配只改变集合初始化语法，不改变机制逻辑。
...
2. 官方对应模块仍为 `Dummy`、`WIP` 或明确不可用时，继续保留 RebornCN 实现。
3. 官方对应模块达到可用成熟度并确认机制完整后，以官方实现替换 RebornCN 文件。
4. 替换后检查 `ReplayValidatedCastAOEs` 和方形拉拽兼容类型是否仍被引用；无引用时再删除。
