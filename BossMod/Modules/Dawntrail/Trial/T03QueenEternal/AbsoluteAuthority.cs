...
sealed class AbsoluteAuthorityFlare(BossModule module) : Components.BaitAwayIcon(module, new AOEShapeCircle(12f), (uint)IconID.Flare, AID.AbsoluteAuthorityFlare, 6f)
{
...
sealed class AbsoluteAuthorityDorito(BossModule module) : Components.GenericStackSpread(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            Stacks.Add(new(actor, 3f, 8, 8, activation: WorldState.FutureTime(5.1f)));
        }
    }
    ...
    public override void Update()
    {
        if (Stacks.Count != 0)
        {
            var party = Raid.WithoutSlot(false, true, true).ToArray();
            ...
            var aoes = _aoe.ActiveAOEs(default, player).ToArray();
