...
sealed class Rush(BossModule module) : Components.GenericBaitAway(module)
{
    ...
    private static readonly AOEShapeCircle _shapeUntethered = new(8);
    ...
    private WPos SafeSpot(Actor source)
    {
        ...
        if (source.Position.Z > 110)
            return Module.Center + new WDir(safeSide * (offX < 6 ? 15 : 10), -19);
        var central = source.Position.Z < 96;
        return Module.Center + new WDir(safeSide * 15, central ? -2 : 10);
    }
}

sealed class IceDart(BossModule module) : Components.BaitAwayTethers(module, new AOEShapeCircle(16), (uint)TetherID.IceDart, AID.IceDart, true)
{
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
...
sealed class RaisedTribute(BossModule module) : Components.GenericWildCharge(module, 4, AID.RaisedTribute, 80)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
