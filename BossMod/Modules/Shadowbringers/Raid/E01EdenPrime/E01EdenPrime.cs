...
class FragorMaximus(BossModule module) : Components.RaidwideCast(module, (uint)AID.FragorMaximus);
class ManaBurst(BossModule module) : Components.RaidwideCast(module, (uint)AID.ManaBurst);
class EdensGravity(BossModule module) : Components.RaidwideCast(module, (uint)AID.EdensGravity);
class DimensionalShift(BossModule module) : Components.RaidwideCast(module, (uint)AID.DimensionalShift);
class ParadisalDive(BossModule module) : Components.ProximityAOEs(module, (uint)AID.ParadisalDive, 20f);
class EdensFlare(BossModule module) : Components.SimpleAOEs(module, (uint)AID.EdensFlare, new AOEShapeDonut(5f, 60f));
class EdensBlizzardIII(BossModule module) : Components.SimpleAOEs(module, (uint)AID.EdensBlizzardIII, new AOEShapeCircle(10f));
class EdensFireIII(BossModule module) : Components.IconStackSpread(module, 0, (uint)IconID.FireIIISpread, 0, (uint)AID.EdensFireIII, 0, 5, 0);
class EdensThunderIII(BossModule module) : Components.SimpleAOEs(module, (uint)AID.EdensThunderIII, new AOEShapeRect(40f, 5f));
class PureBeam(BossModule module) : Components.SimpleAOEs(module, (uint)AID.PureBeam, new AOEShapeRect(60f, 2f));
class ParadiseLost(BossModule module) : Components.SimpleAOEs(module, (uint)AID.ParadiseLost, new AOEShapeCircle(6));
class ViceAndVirtueGravity(BossModule module) : Components.IconStackSpread(module, 0, (uint)IconID.GravitySpread, 0, (uint)AID.ViceOfApathy, 0, 5, 3.2f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (WorldState.CurrentTime < Activation)
        {
            ...
            hints.AddForbiddenZone(new SDCircle(Module.PrimaryActor.Position, 6f), Activation);
            //Spread further than the minimum 5 units to ensure there is a path to run between puddles.
            foreach (var spreadTarget in ActiveSpreads.Select(s => s.Target))
            {
                hints.AddForbiddenZone(new SDCircle(spreadTarget.Position, 6f), Activation);
            }
        }
...
class ViceAndVirtueLines(BossModule module) : Components.BaitAwayTethers(module, new AOEShapeRect(100f, 3f), (uint)TetherID.ViceAndVirtue, (uint)AID.ViceAndVirtueLines);
class GravityPuddle(BossModule module) : Components.VoidzoneAtCastTarget(module, 5, (uint)AID.ViceOfApathy, m => m.Enemies((uint)OID.GravityPuddle).Where(z => z.EventState != 7), 0);
class PureLight(BossModule module) : Components.SimpleAOEs(module, (uint)AID.PureLight, new AOEShapeRect(60f, 60f));
class GuardianOfParadise(BossModule module) : Components.Adds(module, (uint)OID.GuardianOfParadise);
class ManaSlice(BossModule module) : Components.Cleave(module, (uint)AID.ManaSlice, new AOEShapeCone(15, 45.Degrees()), [(uint)OID.GuardianOfParadise]); // TODO: verify angle. Radius definitely larger than 6y, likely 10. 15 for safety
class SpearOfParadise(BossModule module) : Components.SingleTargetCast(module, (uint)AID.SpearOfParadise);
        ...
        if (WorldState.CurrentTime < activation)
        {
            ...
            switch (assignment)
            {
                ...
                case PartyRolesConfig.Assignment.M2:
                    hints.AddForbiddenZone(new SDRect(new WPos(80, 110), new WPos(120, 110), 10f), activation); //don't go to south side.
                    break;
                case PartyRolesConfig.Assignment.R1:
                case PartyRolesConfig.Assignment.H1:
                    hints.AddForbiddenZone(new SDRect(new WPos(110, 80), new WPos(110, 120), 10f), activation); //don't go to east side.
                    hints.AddForbiddenZone(new SDRect(new WPos(80, 90), new WPos(120, 90), 10f), activation); //don't go to north side.
                    break;
                default:
                    hints.AddForbiddenZone(new SDRect(new WPos(90, 80), new WPos(90, 120), 10f), activation); //don't go to west side.
                    hints.AddForbiddenZone(new SDRect(new WPos(80, 90), new WPos(120, 90), 10f), activation); //don't go to north side.
                    break;
            }
        }
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, Contributors = "AndMyAxe", GroupType = BossModuleInfo.GroupType.CFC, GroupID = 653, NameID = 8345)]
