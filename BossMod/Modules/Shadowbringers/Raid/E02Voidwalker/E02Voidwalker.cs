...
class EmptyHateKnockback(BossModule module) : Components.SimpleKnockbacks(module, (uint)AID.EmptyHateKnockback, 25, kind: Kind.DirForward)
{
...
class DarkFireIIINormal(BossModule module) : Components.IconStackSpread(module, 0, (uint)IconID.DarkFireIII, 0, (uint)AID.DarkFireIIINormal, 0, 8, 0);

class UnholyDarknessNormal(BossModule module) : Components.IconStackSpread(module, (uint)IconID.UnholyDarkness, 0, (uint)AID.UnholyDarknessNormal, 0, 6, 0, 0);
...
class ShadowEye(BossModule module) : Components.GenericGaze(module)
{
    ...
    public override ReadOnlySpan<Eye> ActiveEyes(int slot, Actor actor) => ShadowEyeTarget != null ? new Eye[] { new(ShadowEyeTarget.Position, Activation) } : [];
...
class BleedTower(BossModule module) : Components.GenericTowers(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        List<ShapeDistance> zones = [];
        List<ShapeDistance> forbiddenZones = [];
        ...
        foreach (var t in Towers.Where(t => t.Activation <= deadline))
        {
            ...
            {
                soakingPlayers |= Raid.WithSlot().InRadius(t.Position, 2).Mask();
                ...
                if (effNumSoakers < t.MinSoakers || effNumSoakers == t.MinSoakers && t.IsInside(actor))
                {
                    ...
                    zones.Add(new SDCircle(t.Position, 2));
                }
                else if (effNumSoakers > t.MaxSoakers && !haveTowersToSoak)
                {
                    ...
                    forbiddenZones.Add(new SDCircle(t.Position, 2));
            }
        }
        if (zones.Count > 0)
        {
            ShapeDistance zoneUnion = haveTowersToSoak ? new SDInvertedUnion(zones.ToArray()) : new SDUnion(zones.ToArray());
            hints.AddForbiddenZone(zoneUnion, firstActivation);
        }
        if (forbiddenZones.Count > 0)
        {
            var fzu = new SDUnion(forbiddenZones.ToArray());
...
class BleedTowerDanger(BossModule module) : Components.Voidzone(module, 2f, m => m.Enemies((uint)OID.BleedTower).Where(z => z.EventState != 7))
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        foreach (var towerPosition in TowerPositions)
        {
            ...
            {
                aoes.Add(new AOEInstance(Shape, towerPosition.Position));
            }
        }
        return aoes.ToArray();
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Activation > WorldState.CurrentTime)
        {
            ...
            if (WorldState.CurrentTime < preActivation)
            {
                ...
                {
                    hints.AddForbiddenZone(new SDCircle(t.Position, 2f), WorldState.CurrentTime); // void zone, active until it's not, don't try to pass through.
                    hints.AddForbiddenZone(new SDCircle(t.Position, 2f), preActivation); // perhaps this will signal to the AI that it WILL be "safe" to enter? We need the AI to make their way to their tower, even though it's a void zone.
                }
            }
        }
    }
...
class DoomvoidGuillotine(BossModule module) : Components.SimpleAOEs(module, (uint)AID.DoomvoidGuillotine, new AOEShapeRect(50f, 5f));
class EntropyRaidwide(BossModule module) : Components.RaidwideCast(module, (uint)AID.EntropyRaidwide);
class EmptyHateDamage(BossModule module) : Components.RaidwideCast(module, (uint)AID.EmptyHateDamage);
class DoomvoidSlicer(BossModule module) : Components.SimpleAOEs(module, (uint)AID.DoomvoidSlicer, new AOEShapeDonut(5f, 30));
class Shadowflame(BossModule module) : Components.SingleTargetCast(module, (uint)AID.Shadowflame);
class Shadowflame1(BossModule module) : Components.SingleTargetCast(module, (uint)AID.Shadowflame1);

//Nyx radius seems more like 2, but janky
class Nyx(BossModule module) : Components.Voidzone(module, 4f, m => m.Enemies((uint)OID.Nyx).Where(z => z.EventState != 7))
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        var Nyxes = Module.Enemies((uint)OID.Nyx);
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, Contributors = "AndMyAxe", GroupType = BossModuleInfo.GroupType.CFC, GroupID = 684, NameID = 8382)]
