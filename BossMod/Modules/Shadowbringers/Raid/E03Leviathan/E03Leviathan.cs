...
class RipCurrent(BossModule module) : Components.BaitAwayIcon(module, new AOEShapeRect(50f, 3f), (uint)IconID.RipCurrentTarget, (uint)AID.RipCurrentCast, activationDelay: 5.1f, damageType: AIHints.PredictedDamageType.Tankbuster)
{
...
class TemporaryCurrentRight(BossModule module) : Components.SimpleAOEs(module, (uint)AID.TemporaryCurrentRight, new AOEShapeRect(76f, 15f));
class TemporaryCurrentLeft(BossModule module) : Components.SimpleAOEs(module, (uint)AID.TemporaryCurrentLeft, new AOEShapeRect(76f, 15f));
class TidalRoar(BossModule module) : Components.RaidwideCast(module, (uint)AID.TidalRoar);
class Tsunami(BossModule module) : Components.RaidwideCast(module, (uint)AID.Tsunami);

class MonsterWave(BossModule module) : Components.SimpleAOEs(module, (uint)AID.MonsterWave, 6f);

class TidalWave(BossModule module) : Components.SimpleKnockbacks(module, (uint)AID.TidalWave, 20, kind: Kind.DirForward)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
...
class UnderseaQuakeSides(BossModule module) : Components.SimpleAOEs(module, (uint)AID.UnderseaQuakeSides, new AOEShapeRect(40f, 5f));
class UnderseaQuakeMid(BossModule module) : Components.SimpleAOEs(module, (uint)AID.UnderseaQuakeMid, new AOEShapeRect(40f, 10f))
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (WorldState.CurrentTime < Activation)
        {
            ...
            switch (assignment)
            {
                ...
                case PartyRolesConfig.Assignment.R2:
                    hints.AddForbiddenZone(new SDRect(new WPos(90, 80), new WPos(90, 120), 10f), Activation);//don't go left
                    break;
                default:
                    hints.AddForbiddenZone(new SDRect(new WPos(110, 80), new WPos(110, 120), 10f), Activation);//don't go right
                    break;
            }
        }
        ...
        {
            hints.AddForbiddenZone(new SDRect(new WPos(100, 80), new WPos(100, 120), 10f));
        }
        base.AddAIHints(slot, actor, assignment, hints);
    }
}
...
{
    Shape MakePlatform(float offX, float offZ) => new Rectangle(Arena.Center + new WDir(offX, offZ), 5, 20);
    ...
    public override void OnEventDirectorUpdate(uint updateID, uint param1, uint param2, uint param3, uint param4)
    {
        ...
        if (updateID == 0x8000000D)
        {
            ...
            {
                Arena.Bounds = new ArenaBoundsCustom([MakePlatform(15, 0), MakePlatform(-15, 0)]);
            }
        }
    }
}
...
class MaelstromVoidZones(BossModule module) : Components.Voidzone(module, 8f, m => m.Enemies((uint)OID.MaelstromVoidZone))
{
    public List<Actor> voidZoneActors = [];
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor) => voidZoneActors.Select(s => new AOEInstance(Shape, s.Position)).ToArray();
...
class SpinningDive(BossModule module) : Components.SimpleAOEs(module, (uint)AID.SpinningDive, new AOEShapeRect(64f, 10f)); //I think this is the Tsunami portal charges?

class DrenchingPulseSpread(BossModule module) : Components.SpreadFromIcon(module, (uint)IconID.DrenchingPulseSpread, (uint)AID.FreakWave, 5f, 7f);
class CrashingPulseStack(BossModule module) : Components.StackWithIcon(module, (uint)IconID.CrashingPulseStack, (uint)AID.KillerWave, 5f, 7f, 4) //same icon and radius as SmotheringWaters. icon appears ~4 seconds before effect.
{
...
class SurgingWaters(BossModule module) : Components.GenericKnockback(module)
{
    readonly List<(Actor Source, DateTime Activation)> _sources = [];

    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor) => _sources.Select(s => new Knockback(s.Source.Position, 12, s.Activation)).ToArray(); //TODO: verify knockback distance
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 682, NameID = 8486)]
