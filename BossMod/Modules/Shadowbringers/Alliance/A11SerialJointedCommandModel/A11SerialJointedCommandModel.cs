...
class CentrifugalSpin(BossModule module) : Components.SimpleAOEs(module, (uint)AID.CentrifugalSpin1, new AOEShapeRect(30, 4));
class Shockwave(BossModule module) : Components.SimpleKnockbacks(module, (uint)AID.Shockwave, 15);
class HighCaliberLaser(BossModule module) : Components.SimpleAOEs(module, (uint)AID.HighCaliberLaser, new AOEShapeRect(70, 12))
{
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        foreach (var aoe in base.ActiveAOEs(slot, actor))
        {
            ...
            aoes.Add(aoe with { Color = aoe.Activation < group1 ? Colors.Danger : Colors.AOE, Risky = aoe.Activation < group1 });
        }
        return aoes.ToArray();
    }
}
...
{
    private static readonly AOEShapeCapsule _shape = new(2, 8);
    private readonly List<Actor> _balls = [];

    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
        => _balls.Select(ball => new AOEInstance(_shape, ball.Position, ball.Rotation)).ToArray();
...
class HighPoweredLaser(BossModule module) : Components.GenericAOEs(module, (uint)AID.HighPoweredLaser)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
        => Activation == default ? [] : Turrets.Select(t => new AOEInstance(new AOEShapeRect(40, 2), t.Position, t.Rotation, Activation, risky: WorldState.FutureTime(1) > Activation)).ToArray();
...
class SidestrikingSpin(BossModule module) : Components.SimpleAOEGroups(module, [(uint)AID.SidestrikingSpin1, (uint)AID.SidestrikingSpin2], new AOEShapeRect(30, 6));
class EnergyBombardment(BossModule module) : Components.SimpleAOEs(module, (uint)AID.EnergyBombardment1, new AOEShapeCircle(4));
...
class EnergyAssault(BossModule module) : Components.GenericAOEs(module, (uint)AID.EnergyAssault1)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
        => Casters.Select(c => new AOEInstance(new AOEShapeCone(30, 45.Degrees()), c.Item1.Position, c.Item1.CastInfo?.Rotation ?? c.Item1.Rotation, c.Item2)).ToArray();
...
class AirToSurfaceEnergy(BossModule module) : Components.GenericAOEs(module, (uint)AID.AirToSurfaceEnergy)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
        => _predicted.Select((p, i) => p with { Color = i < 8 ? Colors.Danger : Colors.AOE }).Take(AOEsToShow * 8).Reverse().ToArray();
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 700, NameID = 9141)]
