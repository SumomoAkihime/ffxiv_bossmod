...
class Impact(BossModule module) : Components.SimpleAOEs(module, (uint)AID.Impact, new AOEShapeCircle(15));
...
class ConvenientSelfDestruction(BossModule module) : Components.BaitAwayTethers(module, new AOEShapeCircle(5), (uint)TetherID.ConvenientSelfDestruction, (uint)AID.ConvenientSelfDestruction, centerAtTarget: true)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            ShapeDistance[] playerBaits = [.. ImportantBaits(actor).Where(b => b.Target.Role != Role.Tank).Select(b => (ShapeDistance)new SDRect(b.Source.Position, b.Target.Position, 1))];
            if (playerBaits.Length > 0)
            {
                hints.AddForbiddenZone(new SDOutsideOfUnion(playerBaits), Activation);
                return;
            }
        }
    }
