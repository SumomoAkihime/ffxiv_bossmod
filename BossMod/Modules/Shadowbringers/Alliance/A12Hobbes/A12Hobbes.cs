...
class ShockingDischarge(BossModule module) : Components.VoidzoneAtCastTarget(module, 5, (uint)AID.ShockingDischarge, m => m.Enemies(0x1EAEE6).Where(e => e.EventState != 7), 1.03f);
class VariableCombatTestCone(BossModule module) : Components.SimpleAOEGroups(module, [(uint)AID.VariableCombatConeSlow, (uint)AID.VariableCombatConeFast], new AOEShapeCone(20, 30.Degrees()));
class VariableCombatTestCircle(BossModule module) : Components.SimpleAOEGroups(module, [(uint)AID.VariableCombatCircleSlow, (uint)AID.VariableCombatCircleFast], new AOEShapeCircle(2));
class VariableCombatTestDonut(BossModule module) : Components.SimpleAOEGroups(module, [(uint)AID.VariableCombatDonutSlow, (uint)AID.VariableCombatDonutFast], new AOEShapeDonut(7, 19));
...
class ShortRangeMissile(BossModule module) : Components.SpreadFromCastTargets(module, (uint)AID.ShortRangeMissile, 8)
{
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 700, NameID = 9143)]
public class A12Hobbes(WorldState ws, Actor primary) : BossModule(ws, primary, CircleBounds.Center, CircleBounds)
{
    ...
    private static ArenaBoundsCustom MakeBounds() => new([.. PlatformCenters.Select(c => new Circle(c, 20))]);
}
