...
class LaserTurret(BossModule module) : Components.SimpleAOEs(module, (uint)AID.LaserTurret, new AOEShapeRect(85, 5));
class LightfastBlade(BossModule module) : Components.SimpleAOEs(module, (uint)AID.LightfastBlade, new AOEShapeCone(48, 90.Degrees()));
...
class AreaBombingExa(BossModule module) : Components.Exaflare(module, new AOEShapeCircle(4), (uint)AID.BallisticImpact)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == WatchedAction && !Lines.Any(l => l.Rotation == caster.Rotation))
            Lines.Add(new(caster.Position, caster.Rotation.ToDirection() * 4, Module.CastFinishAt(spell), 2, 7, 4, caster.Rotation));
    }
...
class A13GoliathTankStates : StateMachineBuilder
{
    public A13GoliathTankStates(BossModule module) : base(module)
    {
        TrivialPhase()
            ...
            .Raw.Update = () => Module.PrimaryActor.IsDeadOrDestroyed && Module.Enemies((uint)OID.FlightUnit).All(f => f.IsDeadOrDestroyed || f.HPMP.CurHP == 1);
    }
}

[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 700, NameID = 9138)]
public class A13GoliathTank(WorldState ws, Actor primary) : BossModule(ws, primary, new(-780, 555), new ArenaBoundsCircle(30))
{
    protected override void DrawEnemies(int pcSlot, Actor pc)
    {
        ...
        Arena.Actors(Enemies((uint)OID.FlightUnit), Colors.Enemy);
    }
}
