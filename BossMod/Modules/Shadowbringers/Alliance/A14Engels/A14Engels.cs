...
class IncendiaryBombing(BossModule module) : Components.SpreadFromIcon(module, (uint)IconID.IncendiaryBombing, (uint)AID.IncendiaryBombing, 8, 9.1f)
{
...
class IncendiaryBombingVoidzone(BossModule module) : Components.VoidzoneAtCastTarget(module, 8, (uint)AID.IncendiaryBombing, m => m.Enemies((uint)OID.IncendiaryBomb).Where(e => e.EventState != 7), 0.1f);
class DiffuseLaser(BossModule module) : Components.RaidwideCast(module, (uint)AID.DiffuseLaser);

class LaserSight(BossModule module) : Components.GenericAOEs(module, (uint)AID.LaserSight)
{
    private readonly List<Actor> _casters = [];

    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
        => _casters.Select(c => new AOEInstance(new AOEShapeRect(100, 10), c.Position, c.Rotation, Module.CastFinishAt(c.CastInfo))).ToArray();
    ...
    {
        if (spell.Action.ID == WatchedAction || (AID)spell.Action.ID == AID.LaserSightRepeat)
        {
            NumCasts++;
            if (NumCasts >= 5)
            {
...
class SurfaceMissile(BossModule module) : Components.SimpleAOEs(module, (uint)AID.SurfaceMissile, new AOEShapeCircle(6));
...
class ArmLaser(BossModule module) : Components.SimpleAOEs(module, (uint)AID.ArmLaser, new AOEShapeCone(30, 45.Degrees()));
...
class DemolishStructure(BossModule module) : Components.SimpleAOEs(module, (uint)AID.DemolishStructure, new AOEShapeCircle(25));
...
class MarxThrust(BossModule module) : Components.SimpleAOEs(module, (uint)AID.MarxThrust, new AOEShapeRect(30, 10));

class IncendiarySaturationBombing(BossModule module) : Components.SimpleAOEs(module, (uint)AID.IncendiarySaturationBombing, new AOEShapeRect(30, 30));
...
// TODO wheel enrage
class CrushingWheel(BossModule module) : Components.SimpleAOEs(module, (uint)AID.CrushingWheel, new AOEShapeRect(20, 15));
...
[ModuleInfo(BossModuleInfo.Maturity.Contributed, GroupType = BossModuleInfo.GroupType.CFC, GroupID = 700, NameID = 9147)]
