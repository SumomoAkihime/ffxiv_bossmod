...
sealed class Hatch(BossModule module) : Components.CastCounter(module, (uint)AID.Hatch)
{
    ...
    {
        if (Module.PrimaryActor.IsTargetable && hints.FindEnemy(Module.PrimaryActor) is { } twintania)
        {
        ...
        else
        {
            ...
            if (countO > 0)
            {
                ...
                for (var i = 0; i < countO; ++i)
                {
                    ...
                    {
                        hints.AddForbiddenZone(new SDCapsule(orb.Position, orb.LastFrameMovement.ToAngle(), 6f, 2f), WorldState.FutureTime(2d));
                    }
                }
    ...
    private void AssignP1()
    {
        ...
        targets.Sort(static (a, b) => a.Player.InstanceID.CompareTo(b.Player.InstanceID));
