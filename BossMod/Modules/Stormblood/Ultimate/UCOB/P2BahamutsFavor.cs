    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (IsSpreadTarget(actor))
        {
            ...
            if (FirstSet)
            {
                ...
                if (_nael == null || ordered.Length == 0)
                {
                    base.AddAIHints(slot, actor, assignment, hints);
                    return;
                }
                ...
                {
                    if ((uint)Raid.FindSlot(spreads[i].Target.InstanceID) >= (uint)ordered.Length)
                    {
                        base.AddAIHints(slot, actor, assignment, hints);
                        return;
                    }
                ...
                if (actorIndex >= 0)
                {
                    ...
                    for (var i = 0; i < count; ++i)
                    {
                        ...
                        if (assignment2 < myAssignment || assignment2 == myAssignment && i < actorIndex)
                        {
                            ++myOrder;
                        }
                    }
