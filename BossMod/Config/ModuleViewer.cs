...
public sealed class ModuleViewer(PlanDatabase? planDB, WorldState ws) : IDisposable
{
    ...
    private ModuleGroupInfo ClassifyGroup(BossModuleRegistry.Info module)
    {
        ...
        switch (module.GroupType)
        {
            ...
            case BossModuleInfo.GroupType.MaskedCarnivale:
                ...
                return new(Loc.Tr("Stage {0}: {1}", mcSort, FixCase(mcRow.Name)), groupId, mcSort);
            case BossModuleInfo.GroupType.RemovedUnreal:
                return new(Loc.Tr("Removed Content"), groupId, groupId);
            case BossModuleInfo.GroupType.BaldesionArsenal:
                return new(Loc.Tr("Baldesion Arsenal"), groupId, groupId);
            case BossModuleInfo.GroupType.CastrumLacusLitore:
                return new(Loc.Tr("Castrum Lacus Litore"), groupId, groupId);
            case BossModuleInfo.GroupType.TheDalriada:
                return new(Loc.Tr("The Dalriada"), groupId, groupId);
            case BossModuleInfo.GroupType.TheForkedTowerBlood:
                return new(Loc.Tr("The Forked Tower: Blood"), groupId, groupId);
            case BossModuleInfo.GroupType.ForayFATE:
                groupId |= module.GroupID;
                return new(Loc.Tr("{0} FATE", FixCase(Service.LuminaRow<ContentFinderCondition>(module.GroupID)!.Value.Name)), groupId, groupId);
            ...
            case BossModuleInfo.GroupType.Fate:
                return new(Loc.Tr("{0} FATE", module.Expansion.ShortName()), groupId, groupId, _iconFATE);
            case BossModuleInfo.GroupType.Hunt:
                groupId |= module.GroupID;
                return new(Loc.Tr("{0} Hunt {1}", module.Expansion.ShortName(), (BossModuleInfo.HuntRank)module.GroupID), groupId, groupId, _iconHunt);
            case BossModuleInfo.GroupType.CriticalEngagement:
                groupId |= module.GroupID;
                return new(Loc.Tr("{0} CE", FixCase(Service.LuminaRow<ContentFinderCondition>(module.GroupID)!.Value.Name)), groupId, groupId);
            case BossModuleInfo.GroupType.BozjaDuel:
                groupId |= module.GroupID;
                return new(Loc.Tr("{0} Duel", FixCase(Service.LuminaRow<ContentFinderCondition>(module.GroupID)!.Value.Name)), groupId, groupId);
            ...
            case BossModuleInfo.GroupType.GoldSaucer:
                return new(Loc.Tr("Gold saucer"), groupId, groupId);
            default:
                return new(Loc.Tr("Ungrouped"), groupId, groupId);
        }
    }
    ...
    internal readonly struct SupportedFightSortKey(BossModuleInfo.Expansion expansion, BossModuleInfo.Category category, uint groupOrder, int moduleOrder) : IComparable<SupportedFightSortKey>
    {
        ...
        {
            var cmp = ((int)Expansion).CompareTo((int)other.Expansion);
    ...
    internal static SupportedFightSortKey GetSupportedFightSortKey(BossModuleRegistry.Info module)
        => new(module.Expansion, module.Category, SupportedGroupSortOrder(module), module.SortOrder);
    ...
    private static void DrawModuleHelpMarker(BossModuleRegistry.Info info, string helpText)
    {
        ...
        {
            ImGui.TextUnformatted(Loc.Tr(maturityDescription));
        }
