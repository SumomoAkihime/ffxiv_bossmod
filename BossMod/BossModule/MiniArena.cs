...
public sealed class MiniArena(WPos center, ArenaBounds bounds)
{
    ...
    private readonly List<WorldPathCommand> _worldPathCommands = new(32);
    ...
    public void SetWindowSize(Vector2 fullSize)
    {
        ...
        ImGui.SetWindowSize(Vector2.Max(fullSize, currentWindowSize));
    ...
    private WorldProjectionLayerScope WorldProjectionLayerForActor(ref Vector4 posRot)
    {
        ...
        {
            return WorldProjectionLayer(customBounds.ResolveProjectionLayer(new WPos(posRot.X, posRot.Z) - _center, posRot.Y), false, false);
        }
        return default;
    }
    ...
    public void ZoneCircleOutlineUnclipped(WPos center, float radius, uint color = default, float thickness = 1f)
        => AddCircleUnfilled(center, radius, color, thickness);
    ...
    public void IconWorld(Vector3 center, FontAwesomeIcon icon, uint color, float fontSize = 17f)
    {
        ...
        if (!_frameSuppress2DZoneRendering)
        {
            Dx11ArenaRenderer.AppendIconScreen(WorldPositionToScreenPosition(new WPos(center.X, center.Z)), text, fontSize, color);
        }
        if (_frameShowWorldTextIconBillboards)
        {
    ...
    {
        var position = new WPos(posRot.X, posRot.Z);
        var rotation = new Angle(posRot.W);
    ...
    private void Actor(ref Vector4 posRot, uint color, RelSimplifiedComplexPolygon? shape, bool draw2D, bool drawWorld)
    {
        ...
        else
        {
            ...
            ActorOutsideBounds(clamped, new Angle(posRot.W), posRot.Y, color, draw2D, drawWorld);
        }
    }
