...
public sealed class PlanExecution
{
    ...
    public List<PlannedAction> GetUpcomingPlannedActions(WorldState ws, int playerSlot, float lookAheadSeconds = 60)
    {
        ...
        for (var mi = 0; mi < Strategies.Count; ++mi)
        {
            ...
            {
                if (module.Definition.Configs[ti] is not StrategyConfigTrack config || config.AssociatedActions.Count != config.Options.Count - 1)
                    continue;
                ...
                {
                    var e = entries[ei];
                    if (e.WindowEnd < t || e.WindowStart > maxT || !e.IntersectBranchRange(s.BranchID, s.NumBranches))
                        continue;
