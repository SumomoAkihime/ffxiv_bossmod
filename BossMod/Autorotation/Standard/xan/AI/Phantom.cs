...
public class PhantomAI(RotationModuleManager manager, Actor player) : AIBase<PhantomAI.Strategy>(manager, player)
{
    public struct Strategy
    {
        ...
        [Track("Black Mage: DPS", Actions = [PhantomID.OccultFireIII, PhantomID.OccultBlizzardIII, PhantomID.OccultThunderIII, PhantomID.OccultFlare])]
        ...
        [Track("Blue Mage: DPS", Actions = [PhantomID.OccultAero, PhantomID.OccultAeroII, PhantomID.OccultAquaBreath])]
    ...
    private void PRdm(Strategy strategy, Actor? primaryTarget)
    {
        if (strategy.RedMage.IsEnabled() && primaryTarget?.IsAlly == false && !MidCombo)
        {
            ...
            else
            {
                ...
                UseAction(PhantomID.OccultThunderII, primaryTarget, prio + ((weakness & Element.Thunder) != 0 ? 1 : 0), castTime);
            }
        }
    ...
    private void PSmn(Strategy strategy, Actor? primaryTarget)
    {
        ...
        UseAction(PhantomID.Thunderstorm, primaryTarget, prio + ((weakness & Element.Thunder) != 0 ? 1 : 0), 4);
    ...
    public static readonly uint[] BreakableComboStatus = [
        ...
        (uint)BossMod.RDM.SID.Dualcast, // preserve the main job instant-cast combo
