...
public sealed class BLU(RotationModuleManager manager, Actor player) : Castxan<AID, TraitID, BLU.Strategy>(manager, player, PotionType.Intelligence)
{
    ...
    public override void Exec(in Strategy strategy, Enemy? primaryTarget)
    {
        ...
        {
            var (poopTarget, poopNum) = SelectTarget(strategy, primaryTarget, 25, (primary, other) => AIHints.TargetInAOECircle(other, primary.Position, 6));
            if (poopTarget != null && poopNum > 2)
            {
                var scoopNum = Hints.NumPriorityTargetsInAOE(act => StatusDetails(act.Actor, SID.Begrimed, Player.InstanceID).Left > SpellGCDLength && AIHints.TargetInAOECircle(act.Actor, poopTarget.Actor.Position, 6));
                if (scoopNum > 2)
