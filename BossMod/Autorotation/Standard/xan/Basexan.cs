    ...
    where TValues : struct
{
    ...
    {
        var realTarget = ResolveTargetOverride(strategyTrack) ?? defaultTarget;
    ...
    {
        var realTarget = ResolveTargetOverride(strategyTrack.TrackRaw) ?? defaultTarget;
    ...
    protected void GoalZoneCombined(in IStrategyCommon strategy, float range, Func<WPos, float> fAoe, AID firstUnlockedAoeAction, int minAoe, float? maximumActionRange = null)
    {
        ...
        {
            Hints.GoalZones.Add(AIHints.GoalCombined(Hints.GoalSingleTarget(PlayerTarget.Actor, imminent ? positional : Positional.Any, Player, World.Actors, range), fAoe, minAoe));
