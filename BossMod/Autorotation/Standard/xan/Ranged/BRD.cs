...
public sealed class BRD(RotationModuleManager manager, Actor player) : Attackxan<AID, TraitID, BRD.Strategy>(manager, player, PotionType.Dexterity)
{
    ...
    public override void Exec(in Strategy strategy, Enemy? primaryTarget)
    {
        ...
        (BestConeTarget, NumConeTargets) = SelectTarget(strategy, primaryTarget, 12,
            (primary, other) => AIHints.TargetInAOECone(other, Player.Position, 12, Player.DirectionTo(primary), 45.Degrees()));
        ...
        {
            if (CountdownRemaining < ApplicationDelay.Get((uint)AID.Stormbite))
        ...
        var ijDelay = ApplicationDelay.Get((uint)AID.IronJaws);
