...
public sealed class ClassWHMUtility(RotationModuleManager manager, Actor player) : RoleHealerUtility(manager, player)
{
    ...
    public override void Execute(StrategyValues strategy, ref Actor? primaryTarget, float estimatedAnimLockDelay, bool isMoving)
    {
        ...
        if (cureAction != default)
            Hints.ActionsToExecute.Push(ActionID.MakeSpell(cureAction), ResolveTargetOverride(cure.Value) ?? defaultHealTarget, cure.Priority(), cure.Value.ExpireIn, castTime: swift ? 0 : ActionDefinitions.Instance.Spell(cureAction)!.CastTime);
