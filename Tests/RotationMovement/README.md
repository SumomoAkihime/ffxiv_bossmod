...
`RadarDrawingTests` 验证六模块的世界坐标、AOE 更新及清理；`TankFacingTests` 验证坦克朝向目标保持当前距离、拉怪目标共存及关闭移动的语义。棋盘、绝巴哈与模块管理新增专项接入同一入口；PrePullMigrationTests 覆盖青魔斗场战前提示读取、状态机启动和炸弹范围不累积，后续同步须运行完整入口，不能仅凭编译成功验收。
